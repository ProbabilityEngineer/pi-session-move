...
type Change = { kind: "rewrite" | "move"; path: string; target?: string; detail: string };
...
function normalizeText(text: string): { text: string; changed: boolean } {
	if (!home) return { text, changed: false };
	const slashHome = home.replace(/\\/g, "/").replace(/\/+$/g, "");
	const relHome = slashHome.replace(/^\/+/, "");
...
async function rewriteFile(path: string, changes: Change[]): Promise<void> {
	...
	if (apply) await writeFile(path, text, "utf8");
}

async function moveBucket(path: string, changes: Change[]): Promise<void> {
	const bucketCwd = cwdFromSessionBucket(path);
	if (!bucketCwd) return;
	const normalized = collapseDuplicatedHomePrefix(bucketCwd);
	if (normalized === bucketCwd) return;
...
async function main() {
	...
	for (const path of await walk(sessionRoot)) {
		if (basename(path).endsWith(".jsonl")) await rewriteFile(path, changes);
	}
	for (const path of [
	...
	if (!apply) console.log("Run again with --write to apply.");
}
