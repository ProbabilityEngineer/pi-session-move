...
async function restartScriptNames() {
...
async function main() {
    ...
    for (const name of allNames) {
        ...
        const paths = uniq([...(anchor ? descendants(anchor, records) : []), anchor, ...storeNamedPaths].filter(Boolean))
        ...
        if (best)
            rows.push({ name, best, count: infos.length, totalMessages: best.messages });
    }
