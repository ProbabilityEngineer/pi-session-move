...
`pi-move` now owns filesystem repo moves. `pi-relocate` should focus on session relocation, lineage/status, bucket relocation, and prune/recovery UX.

## Acceptance Criteria

- Remove `/relocate-repo` command registration and implementation from `pi-relocate`.
- Remove `/relocate-repos` command registration and implementation if root/repo filesystem moves are moving to `pi-move`.
- Update README/HOWTO command surface to point repo filesystem moves at `pi-move` / `/move <target>`.
